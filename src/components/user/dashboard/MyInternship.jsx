...
export default function MyInternship() {
  ...
  useEffect(() => {
    const fetchInternshipTrack = async () => {
      try {
        ...
        // Extract the first active course from your userModel's populated array
        if (response?.user?.enrolledCourses && response.user.enrolledCourses.length > 0) {
          setActiveCourse(response.user.enrolledCourses[0]);
        }
      } catch (error) {
  ...
  // Fallback UI State if user has not purchased any course yet
  if (!activeCourse) {
    return (
  ...
  const startDateStr = new Date(activeCourse.courseStartDate).toLocaleDateString('en-IN', formatOptions);
  const endDateStr = new Date(activeCourse.courseEndDate).toLocaleDateString('en-IN', formatOptions);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white border border-slate-200/60 rounded-3xl p-6 shadow-sm space-y-6">
          <div className="flex items-start justify-between gap-4">
            ...
            <span className="flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full shrink-0">
        ...
        <div className="bg-white border border-slate-200/60 rounded-3xl p-6 shadow-sm space-y-4 h-fit">
          ...
          <div className="relative border-l-2 border-slate-100 pl-4 space-y-6 py-2">
            ...
            <div className="relative">
              ...
              <span className="block text-[10px] uppercase font-bold text-slate-400">{startDateStr}</span>
              <span className="text-xs sm:text-sm font-bold text-slate-800">Program Launch & Induction</span>
            </div>
            
            {/* Mid-term Benchmark Placeholder */}
            <div className="relative">
              <CheckCircle2 className="w-4 h-4 text-indigo-500 absolute -left-[25px] bg-white rounded-full" />
              <span className="block text-[10px] uppercase font-bold text-slate-400">Evaluations</span>
              <span className="text-xs sm:text-sm font-bold text-slate-800">Mid-Term Synchronization</span>
            </div>
            
            {/* Conclusion */}
            <div className="relative">
              <div className="w-3.5 h-3.5 bg-slate-200 rounded-full absolute -left-[22px] top-0.5 border-2 border-white" />
              <span className="block text-[10px] uppercase font-bold text-slate-400">{endDateStr}</span>
